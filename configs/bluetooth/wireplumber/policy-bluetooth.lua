-- G14-fixes override of WirePlumber 0.4.17's policy-bluetooth.lua
-- (stock file: /usr/share/wireplumber/scripts/policy-bluetooth.lua,
--  sha256 8fe690b14f01e677445f190a32a631d5a5fda6f157e683e0ebf7e80515ab20fa).
--
-- Installed to ~/.config/wireplumber/scripts/ it shadows the stock script.
-- Changes against stock (search for "G14-fixes"):
--   1. Also switch the Bluetooth headset to HSP/HFP when the headset is the
--      *configured default source* (chosen as input device in GNOME), not
--      only when it is the default sink. Lets HDMI/speakers stay the default
--      output while the headset takes the call.
--   2. Ignore a saved "headset profile" that has no input route (e.g.
--      a2dp-sink). Stock remembers whatever profile is active when a call
--      ends and would happily "switch" to a mic-less profile forever.
--
-- WirePlumber
--
...
-- G14-fixes (1): headset chosen as input device counts as "wants the headset"
local function isBluez5AudioSource(source_name)
  if source_name and string.find(source_name, "bluez_input.") ~= nil then
    return true
  end
  return false
end

local function isBluez5ConfiguredDefaultSource()
  local metadata = metadata_om:lookup()
  if not metadata then
    return false
  end
  local configured_source = metadata:find(0, "default.configured.audio.source")
  return isBluez5AudioSource(configured_source)
end
...
local function switchProfile()
  ...
  for device in devices_om:iterate() do
    ...
    if saved_headset_profile then
      ...
      -- G14-fixes (2): never "switch" to a remembered profile without a mic
      if index ~= INVALID and not hasProfileInputRoute(device, index) then
        Log.info("Saved headset profile '" .. saved_headset_profile
              .. "' has no input route, ignoring it")
        index = INVALID
      end
    end
...
local function checkStreamStatus(stream)
  ...
  -- G14-fixes (1): default sink OR configured default source is the headset
  if not (isBluez5DefaultAudioSink() or isBluez5ConfiguredDefaultSource()) then
    return false
  end
...
metadata_om:connect("object-added", function (_, metadata)
  metadata:connect("changed", function (m, subject, key, t, value)
    if (use_headset_profile and subject == 0 and
        ((key == "default.audio.sink" and isBluez5AudioSink(value)) or
         -- G14-fixes (1): also rescan when the headset becomes the chosen input
         (key == "default.configured.audio.source" and isBluez5AudioSource(value)))) then
      -- If bluez sink/source is set as default, rescan for active input streams
      handleAllStreams()
    end
  end)
end)
